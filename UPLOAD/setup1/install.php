  ...
  if (isset($_POST['db_action'])) {
      ...
      if (!$err) {
        ...
        <em>The installation can continue, but the site will be empty, without any simple information, categories or items.</em>";
          ...
          mysqli_query($link, "INSERT INTO `users` (username,password,email,fname,lname,created,userlevel,active)
        VALUES ('" . sanitize($user) . "','" . sha1($pass) . "','" . sanitize($site_email) . "','Web', 'Master',NOW(),'9','y')");
