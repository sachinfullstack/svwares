      ...
      public function processQuestion()
      {
          ...
          if (empty(Filter::$msgs)) {
			  ...
			  if( $qtype == 2 ){
				...
				foreach($_FILES["answer5"]["name"] as $a => $b){
                                    
                                   $target_dir = "../images/";
                                   $new_check = $_FILES["answer5"]["name"][$a];
                                   $new_temp = $_FILES["answer5"]["tmp_name"][$a];
                                   
                                $target_file = $target_dir.basename($new_check);
                                move_uploaded_file($new_temp,$target_file);
                                        $data = array(
						'question' => intval($lastid), 
						'answer' => $target_file,
						'correct' => intval($correct5[$a])
					);
					self::$db->insert("answers", $data);
				  }
			  }
	  ...
	  public function updateQuestion()
      {
          ...
          if (empty(Filter::$msgs)) {
			  ...
			  if( $utype == 1 ){
				...
				$answer5 = $_POST['answer5'];
				$ansid5 = $_POST['ansid5'];
				$banned5 = $_POST['banned5'];
				
				foreach($answer5 as $a => $b){
					$data = array( 
						'answer' => $answer5[$a],
						'correct' => $correct5[$a],
						'banned' => $banned5[$a]
					);
					self::$db->update("answers", $data, "id='" . $ansid5[$a] . "'");
				}
			  }
			  else if( $utype == 4 ){
