		...
		<form id="ques_form" class="xform" method="post">
			<div class="row" id="ques_area">
				<section class="col col-12">
					...
					<h3><?php echo htmlspecialchars_decode($description); ?></h3>
					...
					<?php foreach ($arow as $answers):?>
						...
						<?php } else if($qrow->type == 2){?>
							<label class="radio <?php if($answers->correct == 1){ echo "correct-class";} ?>">
							<input type='radio' name='marked' disabled="disabled" value='<?php echo $answers->correct;?>' /><i></i><?php echo $k; ?>) <?php echo htmlspecialchars_decode($answers->answer); ?></label>
						...
						<?php } else if($qrow->type == 4){?>
							<label class="input <?php echo "correct-class"; ?>">
								Answer: <?php echo htmlspecialchars_decode($answers->answer);?>
							</label>
						<?php } ?>
					...
					<?php $tansrow = $user->yourAnswer($qrow->id);
							...
							if($qrow->type == 1 && $tansrow->marked == 1){
								...
								echo 'Answer: ' . htmlspecialchars_decode($tansrow->correct);
							}else if($qrow->type == 2){

								echo '<label class="radio"><input type="radio" name="ans" value="1" disabled="disabled" /><i></i>' . $m . ') ' . htmlspecialchars_decode($tansrow->answer) . '</label>';
