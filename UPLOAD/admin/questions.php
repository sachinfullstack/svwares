...
<form class="xform" id="admin_form" method="post">
    ...
    <div class="row">
      ...
      <section class="col col-4">
        <div class="radio-toolbar" id="tabs">
          ...
          <input name="qtype" type="radio" id="radio5" value="<?php echo $row->type;?>" /><label for="radio2">Pictures</label>
		  <?php } ?>
        </div>
      </section>
    </div>
  ...
  <div id="general" class="tab_content">
	...
	<div class="row">	  
	  <?php foreach ($arow as $answers):?>
	  <input name="ansid5[]" type="hidden" value="<?php echo $answers->id;?>" />
	  <section class="col col-9">
		<div class="field-wrap">
                    <input type="file" name="answer5[]" id="fileToUpload">
			<!--<textarea class="post" name="answer5[]" rows="5"><?php echo $answers->answer; ?></textarea>-->
		</div>
      </section>
	  <?php
	  if($answers->correct == 1){
...
<?php $ptype = $content->getExamTypes();?>
<form class="xform" id="admin_form" method="post" enctype="multipart/form-data" >
  <div class="subheader">
    <div class="row">
      ...
      <section class="col col-12">
        <div class="radio-toolbar" id="tabs">
          ...
          <input name="qtype" type="radio" id="radio5" value="5" /><label for="radio5">Pictures</label>
        </div>
      </section>
    </div>
  </div>
  <div id="general" class="tab_content">
	...
	<div class="row" id="qtypea4">	  
	  <section class="col col-12">
        ...
        <div class="row input_fields_wrap_media" id="qtypea5">	  
	  <section class="col col-12">
        <button class="button add_field_button_media" style="float: left;">Add More Answer</button>
      </section>
	  <section class="col col-9">
		<div class="field-wrap">
                     <input type="file" name="answer5[]" id="fileToUpload">
			<!--<textarea class="post" name="answer5[]" rows="5"></textarea>-->
...
$(document).ready(function() {
    ...
    var wrapper_media         = $(".input_fields_wrap_media"); //Fields wrapper
    var add_button      = $(".add_field_button"); //Add button ID
    var add_button_media      = $(".add_field_button_media"); //Add button ID
    ...
    $(add_button_media).click(function(e){ //on add input button click
        e.preventDefault();
        if(x < max_fields){ //max input box allowed
            x++; //text box increment
            $(wrapper_media).append('<section class="col col-9"> <div class="field-wrap">  <input type="file" name="answer5[]" id="fileToUpload"> </div> </section> <section class="col col-3"> <select name="correct5[]"> <option value="0">Incorrect</option> <option value="1">Correct</option> </select> </section>'); //add input box
        }
    });
  ...
  <div class="content2">
    ...
    <table class="myTable">
      ...
      <tr>
		...
		<span class="tbicon"> <a href="index.php?do=questions&amp;action=edit&amp;id=<?php echo $row->id;?>" class="tooltip" data-title="<?php echo lang('EDIT').': '.strip_tags($row->description);?>"><i class="icon-pencil"></i></a> </span>
        <?php if($user->userlevel == 9):?>
        <span class="tbicon"> <a href="javascript:void(0);" id="item_<?php echo $row->id;?>" class="tooltip delete" data-rel="<?php echo strip_tags($row->description);?>" data-title="<?php echo lang('DELETE').': '.strip_tags($row->description);?>"><i class="icon-trash"></i></a> </span>
        <?php endif;?></td>
      </tr>
      <?php endforeach;?>
      <?php unset($row);?>
